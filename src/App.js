...
const Centers = ({ districtId, centers, setCenters, setIsLoading, setLastUpdated }) => {
  ...
  let availableCenters = centers.filter(c => {
    return c.sessions.some(s => (s.min_age_limit >= 45 && s.available_capacity !== 0)) && c.fee_type === 'Paid'
  }).map(center => {
  ...
  if (availableCenters.length === 0) {
    return <Center>
      <Text>No private hospital slots available for 45+ persons in this district</Text>
    </Center>
  }
